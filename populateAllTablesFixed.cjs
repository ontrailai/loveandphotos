...
/**
 * Populate All Tables Script (Fixed)
 *
...
async function populatePackages() {
  ...
  const packages = [];
  const packageTypes = [
    { name: 'Basic Photography', duration: 120, deliverables: 50 },
    { name: 'Standard Package', duration: 240, deliverables: 100 },
    { name: 'Premium Coverage', duration: 480, deliverables: 200 },
    { name: 'Full Day Package', duration: 600, deliverables: 300 },
    { name: 'Mini Session', duration: 60, deliverables: 25 },
    { name: 'Engagement Session', duration: 120, deliverables: 75 },
    { name: 'Wedding Package', duration: 480, deliverables: 400 },
    { name: 'Event Coverage', duration: 360, deliverables: 150 }
  ];
  ...
  for (const photographer of photographers) {
    ...
    for (let i = 0; i < numPackages; i++) {
      ...
      packages.push({
        ...
        base_price: basePrice * (packageType.duration / 120), // Price based on duration
        duration_minutes: packageType.duration,
        deliverables: {
          photos: packageType.deliverables,
          edited: true,
          digital_delivery: true,
          print_release: packageType.duration >= 240
        },
        includes: [
          `${packageType.duration / 60} hours of coverage`,
          `${packageType.deliverables} edited photos`,
          'Online gallery',
          'Print release',
          packageType.duration >= 240 ? 'Second shooter' : null,
          packageType.duration >= 480 ? 'Album included' : null
        ].filter(Boolean),
        upsells: {
          extra_hour: basePrice,
          rush_delivery: basePrice * 0.5,
          raw_files: basePrice * 0.3
        },
        max_guests: packageType.duration >= 360 ? 200 : 50,
...
async function populateBookings() {
  ...
  if (!customers || customers.length === 0) {
    console.log('⚠️  No customers found, creating some customer users first');
    ...
    if (newCustomers) {
      ...
      console.log('⚠️  Could not create customers, skipping bookings');
      return;
    }
  }
  ...
  for (let i = 0; i < numBookings; i++) {
    ...
    bookings.push({
      ...
      status: status,
      total_price: packageData.base_price,
      deposit_paid: status !== 'pending' ? packageData.base_price * 0.3 : 0,
...
async function populateReviews() {
  ...
  const reviewTexts = [
    ...
    "Exceptional quality and service. Will book again!",
    "Creative vision and flawless execution.",
    "Patient with kids and captured amazing candid moments.",
    "Professional setup and beautiful final products.",
    "Worth every penny! The photos are treasures."
  ];
...
async function populateMessages() {
  ...
  const messageTemplates = [
    ...
    "The meeting was great, let's proceed with the booking.",
