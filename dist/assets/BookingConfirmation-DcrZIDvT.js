import{j as e,C as y,f as q,P as L,d as R,i as U,M as F,n as V,A as Q,K as H,D as O,N as G}from"./ui-D6K4NDfc.js";import{r as x}from"./react-C3Xvg9zH.js";import{u as J,b as K,L as P}from"./router-DqlHMIBt.js";import{s as m,u as W,C as v,a as b,B as g}from"./index-C3rg1TAC.js";import{f as p}from"./utils-CWWonWgh.js";import{z as u}from"./notifications-AAMUpyrg.js";import"./auth-BAx15x3q.js";import"./forms-BETPFrzU.js";const k=async({bookingId:t,recipientType:o})=>{try{const{data:a,error:l}=await m.from("bookings").select(`
        *,
      ...
      `).eq("id",t).single();if(l)throw l;const i=o==="customer"?{email:a.customer.email,name:a.customer.full_name,type:"customer"}:{email:a.photographers.users.email,name:a.photographers.users.full_name,type:"photographer"},d=o==="customer"?X(a):Z(a),{data:s,error:h}=await m.functions.invoke("send-email",{body:{to:i.email,subject:d.subject,html:d.html,text:d.text}});return h&&(console.error("Email send error:",h),await ee({bookingId:t,recipientType:o,recipient:i,content:d})),{success:!0,data:s}}catch(a){throw console.error("Error sending confirmation email:",a),a}},X=t=>{const o=p(new Date(t.event_date),"EEEE, MMMM dd, yyyy"),a=t.contract_url||"#",l="http://localhost:5173/dashboard",i=`http://localhost:5173/book/quiz/${t.id}`,d=`Booking Confirmed - ${t.photographers.users.full_name} for ${o}`,s=`
    <!DOCTYPE html>
    <html>
    <head>
      <style>
...
View in Job Queue: ${a}
  `;return{subject:l,html:i,text:d}},ee=async({bookingId:t,recipientType:o,recipient:a,content:l})=>{try{const{error:i}=await m.from("email_queue").insert({booking_id:t,recipient_type:o,recipient_email:a.email,recipient_name:a.name,subject:l.subject,html_content:l.html,text_content:l.text,status:"pending",attempts:0,created_at:new Date().toISOString()});if(i)throw i}catch(i){console.error("Error queuing email:",i)}},ce=()=>{const{user:t,profile:o}=W(),a=J(),[l]=K(),i=l.get("session_id"),d=l.get("booking_id"),[s,h]=x.useState(null),[D,M]=x.useState(!0),[S,w]=x.useState(!1),[j,N]=x.useState("");x.useEffect(()=>{i?z():d?_(d):(u.error("Invalid confirmation link"),a("/dashboard"))},[i,d]);const z=async()=>{try{const n=await(await fetch(`/api/verify-payment?session_id=${i}`)).json();if(!n.success)throw new Error("Payment verification failed");const c=n.metadata?.bookingId;if(!c)throw new Error("Booking ID not found");const{error:f}=await m.from("bookings").update({payment_status:"paid",booking_status:"confirmed",stripe_session_id:i,updated_at:new Date().toISOString()}).eq("id",c);if(f)throw f;await _(c),await B(c)}catch(r){console.error("Error verifying payment:",r),u.error("Failed to verify payment"),a("/dashboard")}},_=async r=>{try{const{data:n,error:c}=await m.from("bookings").select(`
          *,
          photographers (
            *,
            users!inner (
        ...
        `).eq("id",r).eq("customer_id",t.id).single();if(c)throw c;if(!n){u.error("Booking not found"),a("/dashboard");return}h(n),n.contract_url?N(n.contract_url):await T(n)}catch(n){console.error("Error loading booking:",n),u.error("Failed to load booking details")}finally{M(!1)}},T=async r=>{try{const n=A(r),c=new Blob([n],{type:"text/html"}),f=new File([c],`contract-${r.id}.html`),$=`contracts/${r.id}/contract.html`,{data:te,error:C}=await m.storage.from("contracts").upload($,f,{contentType:"text/html",upsert:!0});if(C)throw C;const{data:{publicUrl:E}}=m.storage.from("contracts").getPublicUrl($);N(E),await m.from("bookings").update({contract_url:E,updated_at:new Date().toISOString()}).eq("id",r.id)}catch(n){console.error("Error generating contract:",n)}},A=r=>`
      <!DOCTYPE html>
      <html>
      <head>
