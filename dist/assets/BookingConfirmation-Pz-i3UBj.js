import{j as e,q as y,d as q,P as L,n as R,v as U,c as F,x as V,A as Q,a9 as H,W as O,aa as G}from"./ui-NsMHn6hE.js";import{r as x}from"./react-DaXN_vW6.js";import{u as J,b as W,L as P}from"./router-CP0DyCRA.js";import{s as m,u as X,C as v,a as b,B as g}from"./index-DOjQlh-D.js";import{f as p}from"./utils-CaoaIqjo.js";import{z as u}from"./notifications-CxCccQbj.js";import"./auth-CAO6bjLU.js";import"./forms-BJUWFeKa.js";const k=async({bookingId:t,recipientType:o})=>{try{const{data:a,error:l}=await m.from("bookings").select(`
        *,
      ...
      `).eq("id",t).single();if(l)throw l;const i=o==="customer"?{email:a.customer.email,name:a.customer.full_name,type:"customer"}:{email:a.photographers.users.email,name:a.photographers.users.full_name,type:"photographer"},d=o==="customer"?K(a):Z(a),{data:s,error:h}=await m.functions.invoke("send-email",{body:{to:i.email,subject:d.subject,html:d.html,text:d.text}});return h&&(console.error("Email send error:",h),await ee({bookingId:t,recipientType:o,recipient:i,content:d})),{success:!0,data:s}}catch(a){throw console.error("Error sending confirmation email:",a),a}},K=t=>{const o=p(new Date(t.event_date),"EEEE, MMMM dd, yyyy"),a=t.contract_url||"#",l="https://loveandphotos.onrender.com/dashboard",i=`https://loveandphotos.onrender.com/book/quiz/${t.id}`,d=`Booking Confirmed - ${t.photographers.users.full_name} for ${o}`,s=`
    <!DOCTYPE html>
    <html>
    <head>
      <style>
...
Questions? Contact us at support@loveandphotos.com
  `;return{subject:d,html:s,text:h}},Z=t=>{const o=p(new Date(t.event_date),"EEEE, MMMM dd, yyyy"),a="https://loveandphotos.onrender.com/dashboard/photographer/job-queue",l=`New Booking Alert - ${t.customer.full_name} on ${o}`,i=`
    <!DOCTYPE html>
    <html>
    <head>
      <style>
...
//# sourceMappingURL=BookingConfirmation-Pz-i3UBj.js.map
