...
/**
 * Populate Remaining Tables Script (Final Version)
 *
...
async function populateBookings() {
  ...
  // Fetch ALL users from the users table (not filtering by role since they're all customers)
  ...
  console.log(`📊 Found ${photographers?.length || 0} photographers, ${users?.length || 0} users, ${packages?.length || 0} packages`);
  ...
  for (let i = 0; i < bookingCount; i++) {
    ...
    const totalAmount = packageData.base_price || Math.floor(Math.random() * 3000) + 500;
  ...
  for (let i = 0; i < bookings.length; i += batchSize) {
    ...
    if (error) {
      ...
      console.log(`  ✅ Batch ${Math.floor(i / batchSize) + 1}: ${data.length} bookings`);
    }
  }

  console.log(`✅ Total inserted: ${inserted} bookings`);
  return inserted;
}
