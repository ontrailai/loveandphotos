...
const AddOnsDetails = () => {
  ...
  const handleContinue = async () => {
    try {
      const addonsData = { selectedAddons: selectedAddons }

      // Update booking context with add-ons details
      updateAddonsDetails(addonsData)

      // Proceed to account setup for authentication & booking creation
      navigate(`/booking/${photographerId}/account`)
    } catch (error) {
      console.error('❌ Failed to continue from add-ons:', error)
      alert('Something went wrong while saving your add-ons. Please try again.')
    }
  }
  ...
  const handleSkip = async () => {
    try {
      ...
      // Move ahead to account setup so user can authenticate & finalize booking
      navigate(`/booking/${photographerId}/account`)
    } catch (error) {
      console.error('❌ Failed to skip add-ons:', error)
      alert('Something went wrong while skipping add-ons. Please try again.')
    }
  }
